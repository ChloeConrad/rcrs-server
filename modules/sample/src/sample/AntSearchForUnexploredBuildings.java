...
public final class AntSearchForUnexploredBuildings{
	private Map<EntityID, Set<EntityID>> graph;
	private Set<EntityID>                buildingSet;
	
	public AntSearchForUnexploredBuildings( StandardWorldModel world) {
	...
	public List<EntityID> antSearch( EntityID start,
		    ...
		    do
		    {
		    	...
		    	for (EntityID voisin : voisins) {
		    		presP=presP+voisin.getPheromoneBuildings();
		    	}
		    	...
		    	//Pas compris l'algo
		    		...
		    		for(int i=0;i<probV.length;i++) {
		    			probV[i]=voisinsTAB[i].getPheromoneBuildings()/presP;
		    		}
