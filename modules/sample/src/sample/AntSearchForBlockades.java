...
public final class AntSearchForBlockades{
	private Map<EntityID, Set<EntityID>> graph;
	private Set<EntityID>                buildingSet;
	
	public AntSearchForBlockades( StandardWorldModel world) {
	...
	public List<EntityID> antSearch( EntityID start,
		    ...
		    do
		    {
		    	...
		    	for (EntityID voisin : voisins) {
		    		presP=presP+voisin.getPheromoneBlockades();
		    	}
		    	...
		    	//Pas compris l'algo
		    		...
		    		for(int i=0;i<probV.length;i++) {
		    			probV[i]=voisinsTAB[i].getPheromoneBlockades()/presP;
		    		}
